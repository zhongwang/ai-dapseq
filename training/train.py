...
GENE_PAIRS_TSV_PATH = "../data_processing/gene_pairs_coexpression.tsv"
FEATURE_VECTOR_DIR = "../data_processing/feature_vectors/"
MODEL_SAVE_PATH = "best_siamese_model.pth" # Added

# --- Model Hyperparameters (Example values, tune as needed) ---
INPUT_FEATURE_DIM = 84      # Example: 4 (DNA one-hot) + 80 (TF affinities)
...
MAX_SEQ_LEN = 2000          # Max promoter sequence length for positional encoding
...
class GenePairDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        gene1_id = self.gene_pairs.iloc[idx]['gene1_id']
        gene2_id = self.gene_pairs.iloc[idx]['gene2_id']
        correlation = float(self.gene_pairs.iloc[idx]['co_expression_correlation'])

        try:
            promoter_seq1_path = os.path.join(self.feature_dir, f"{gene1_id}.npy")
            promoter_seq2_path = os.path.join(self.feature_dir, f"{gene2_id}.npy")
            promoter_seq1 = np.load(promoter_seq1_path)
            promoter_seq2 = np.load(promoter_seq2_path)
        except FileNotFoundError as e:
            print(f"Error loading .npy file for gene pair: ({gene1_id}, {gene2_id}) at index {idx}.")
...
def split_data_gene_disjoint(all_gene_pairs_df, train_frac=0.7, val_frac=0.15, random_state=42):
    ...
    genes2 = all_gene_pairs_df['gene2_id'].unique()
    ...
    train_pairs_df = all_gene_pairs_df[all_gene_pairs_df['gene1_id'].isin(train_genes_set) & all_gene_pairs_df['gene2_id'].isin(train_genes_set)]
    val_pairs_df = all_gene_pairs_df[all_gene_pairs_df['gene1_id'].isin(val_genes_set) & all_gene_pairs_df['gene2_id'].isin(val_genes_set)]
    test_pairs_df = all_gene_pairs_df[all_gene_pairs_df['gene1_id'].isin(test_genes_set) & all_gene_pairs_df['gene2_id'].isin(test_genes_set)]
...
if __name__ == '__main__':
    ...
    if os.path.exists(GENE_PAIRS_TSV_PATH) and os.path.isdir(FEATURE_VECTOR_DIR):
        ...
        try:
            ...
            required_cols = ['gene1_id', 'gene2_id', 'co_expression_correlation']
    ...
    if not actual_data_mode:
        ...
        gene1_ids = np.random.choice(dummy_gene_pool, num_dummy_pairs)
        gene2_ids = np.random.choice(dummy_gene_pool, num_dummy_pairs)
        valid_pairs = []
        seen_pair_hashes = set()
        for g1, g2 in zip(gene1_ids, gene2_ids):
            if g1 == g2: continue
            pair_hash = tuple(sorted((g1, g2)))
            if pair_hash not in seen_pair_hashes:
                valid_pairs.append({'gene1_id': g1, 'gene2_id': g2, 'co_expression_correlation': np.random.rand()})
        ...
        if all_pairs_df.empty: # Fallback
             all_pairs_df = pd.DataFrame({
                'gene1_id': [f'g{i}' for i in range(10)], 'gene2_id': [f'g{i+10}' for i in range(10)],
             ...
             all_pairs_df = pd.concat([all_pairs_df, pd.DataFrame({
                 'gene1_id': ['g0', 'g1', 'g2'], 'gene2_id': ['g1', 'g2', 'g0'], 
        ...
        all_genes_in_dummy_df = set(all_pairs_df['gene1_id']).union(set(all_pairs_df['gene2_id']))
    ...
    if train_df.empty:
        ...
        scheduler = None
        if val_loader:
            scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=LR_SCHEDULER_PATIENCE, factor=0.1, verbose=True)
