...
def parse_arguments():
    ...
    parser.add_argument("--coexpression_file", required=True, help="Path to the input TSV file with gene pairs and correlations (e.g., gene1_id, gene2_id, correlation_coefficient).")
...
def prepare_coexpression_data(coexpression_file_path, output_file_path, processed_signals_dir_path=None, validate_genes_flag=False):
    ...
    try:
        ...
        # Expecting columns like 'gene1_id', 'gene2_id', 'correlation_coefficient'
        # Adjust column names if they are different in the actual input file.
        expected_cols = ['gene1_id', 'gene2_id', 'correlation_coefficient'] # Example names
    ...
    if validate_genes_flag:
        ...
        # Assuming column names are 'gene1_id' and 'gene2_id'
        # Adjust if your column names are different
        try:
            mask_gene1_exists = coexp_df['gene1_id'].apply(check_gene_processed)
            mask_gene2_exists = coexp_df['gene2_id'].apply(check_gene_processed)
            ...
            print(f"Error during validation: Column {e} not found in the co-expression file. Please ensure column names are 'gene1_id' and 'gene2_id' or adjust script.")
            return
        except Exception as e:
